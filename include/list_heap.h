...
#include <type_traits>
#include <utility>
template <typename T, typename Compare = std::less<T>> struct list_heap {
private:
...
public:
  ...
  [[nodiscard]] int size() const { return list_for_heap.size(); }

  [[nodiscard]] bool empty() const { return list_for_heap.empty(); }
